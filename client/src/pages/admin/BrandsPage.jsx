...
import { useGetBrandsQuery, useCreateBrandMutation, useDeleteBrandMutation } from "../../features/api/apiSlice";
...
export default function BrandsPage() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert(err?.data?.message || "Failed to add brand");
    }
  };
  ...
  const handleDelete = async (b) => {
    if (window.confirm(`Are you sure you want to delete ${b.name}?`)) {
      try {
        await deleteBrand(b.slug).unwrap();
        alert("Brand deleted!");
      } catch (err) {
        console.error(err);
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mt-6">
        <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Add New Brand</h3>
      ...
      <div className="mt-8">
        <h3 className="text-lg font-bold mb-4">All Brands</h3>
        {isLoading ? (
          ...
          <p className="text-gray-500 italic">No brands found. Create one above.</p>
        ) : (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 divide-y divide-gray-100">
            {brands.map((b) => (
              <div key={b._id} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
                <div>
                  <div className="font-bold text-gray-900">{b.name}</div>
                  <div className="text-sm text-gray-500 font-mono mt-1">{b.slug}</div>
                </div>
                <div>
                  <button 
